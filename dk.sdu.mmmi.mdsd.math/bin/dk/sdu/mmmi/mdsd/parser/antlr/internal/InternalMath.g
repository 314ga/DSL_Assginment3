...
// Entry rule entryRuleExternalVar
entryRuleExternalVar returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getExternalVarRule()); }
	iv_ruleExternalVar=ruleExternalVar
	{ $current=$iv_ruleExternalVar.current; }
	EOF;

// Rule ExternalVar
ruleExternalVar returns [EObject current=null]
@init {
	enterRule();
}
@after {
...
@after {
	...
	(
		...
		{
			$current = $this_ExternalVar_5.current;
			afterParserOrEnumRuleCall();
		}
	)
;
